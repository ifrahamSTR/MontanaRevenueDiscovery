...
# REPO_ROOT is .../Tools/STRClusterDiscovery/Montana/webpage; Snowflake/ lives
# at the STRWork root, four levels above REPO_ROOT.
INPUT_CSV = REPO_ROOT.parent.parent.parent.parent / "Snowflake" / "Montana" / "Iqbal_Montana_2026-09-18-0155.csv"
...
# ---------------------------------------------------------------------------
# Region-discovery tuning. Re-tuned for Montana: its high-revenue listings
# are far more sparsely distributed across a much larger state than Idaho or
# Utah, so a looser eps is needed just to let real, contiguous markets form
# a cluster at all -- verified by inspecting cluster city composition at
# eps=10/12/15/20/25km before settling on eps=15km, min_samples=4 (the same
# min_samples as Idaho/Utah; smaller clusters like Billings/Red Lodge/Missoula
# still form fine at n=4-8). MERGE_KM=35 was chosen by computing pairwise
# centroid distances between all resulting clusters: the Paradise Valley
# (Emigrant/Livingston/Pray) and Gardiner clusters sit only ~33km apart --
# almost certainly one contiguous Yellowstone-River corridor split by a gap
# in >=$90k listings -- while every other pair of distinct markets (e.g.
# Big Sky-to-Bozeman, Gardiner-to-West Yellowstone) sits at 49km or farther,
# so 35km merges the former without accidentally fusing the latter.
# ---------------------------------------------------------------------------
REGION_REVENUE_THRESHOLD = 90000  # matches js/config.js CONFIG.defaultThreshold
DBSCAN_EPS_KM = 15
DBSCAN_MIN_SAMPLES = 4
MERGE_KM = 35  # union clusters whose centroids are closer than this
...
# Montana-specific: dominant-city -> (slug, display name). A cluster is named
...
REGION_LABELS = {
    ...
    "Whitefish": ("flathead-valley-glacier", "Flathead Valley & Glacier National Park"),
    "Big Sky": ("big-sky", "Big Sky"),
    "Emigrant": ("paradise-valley-yellowstone-north", "Paradise Valley & Yellowstone North"),
    "Livingston": ("paradise-valley-yellowstone-north", "Paradise Valley & Yellowstone North"),
    "Gardiner": ("paradise-valley-yellowstone-north", "Paradise Valley & Yellowstone North"),
    "Missoula": ("missoula", "Missoula"),
    "Red Lodge": ("red-lodge", "Red Lodge"),
